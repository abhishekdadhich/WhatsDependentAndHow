...
namespace WhatsDependentAndHow.Commands
{
    public class ButtonGenerateTreeCommand : ICommand
    {
        ...
        private void LoadTree()
        {
            ...
            {
                _treeGeneratorViewModel.UpdateStatus("xlApp or xlWorkbook is null. Can't proceed.");
                return;
            }
            ...
            try
            {
                ...
                _treeGeneratorViewModel.UpdateStatus(string.Format("Generating {0} Tree for {1}!{2}...", _mode, selectedExcelWorkSheet.Name, selectedExcelCell.Address[false, false]));
                ...
                if (_incompleteProcessing)
                    _treeGeneratorViewModel.UpdateStatus(string.Format("At least one path is not processed completely. See log for details. Total nodes processed: {0}. Total time taken: {1}", _totalNodesInTree, stopWatch.Elapsed));
                else
                    _treeGeneratorViewModel.UpdateStatus(string.Format("All processing complete. Total nodes processed: {0}. Total time taken: {1}.", _totalNodesInTree, stopWatch.Elapsed));
        ...
        private bool CheckTimeoutAndTotalNodeLimit(int minutes)
        {
            ...
            {
                _treeGeneratorViewModel.UpdateStatus("Timeout of " + _timeOutInMinutes + " reached. NOTE: Exiting with incomplete processing.");
                _incompleteProcessing = true;
                return false;
            }
            ...
            {
                _treeGeneratorViewModel.UpdateStatus("Processed maximum allowed node: " + _allWorkBookCellObjects.Count + ". NOTE: Exiting with incomplete processing.");
                _incompleteProcessing = true;
                return false;
            }
        ...
        private bool ValidateInput(Excel.Worksheet selectedWorkSheet, Excel.Range selectedCell)
        {
            ...
            {
                _treeGeneratorViewModel.UpdateStatus("Please select Sheet Name from Combo and Enter Cell Address in Text Box.");
                return false;
            }

            if(selectedWorkSheet == null)
            {
                _treeGeneratorViewModel.UpdateStatus("Selected Sheet not found in Excel Workbook. Can't proceed.");
                return false;
            }

            if(selectedCell == null || selectedCell.Value2 == null || selectedCell.Value2.ToString().Trim().Length == 0)
            {
                _treeGeneratorViewModel.UpdateStatus("Entered Cell Address doesn't exist or it is blank. Can't proceed.");
                return false;
            }

            if(string.IsNullOrEmpty(_mode))
            {
                _treeGeneratorViewModel.UpdateStatus("Command Parameter is blank - code needs to be checked. Can't Proceed.");
                return false;
            }
